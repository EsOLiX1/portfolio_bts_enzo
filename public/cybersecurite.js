...
function afficherArticle(article, id) {
  ...
  // Toujours append (ordre déjà trié en amont)
  ...
  try {
    ...
    querySnapshot.forEach(doc => {
      ...
      if (data.createdAt && typeof data.createdAt.toMillis === 'function') {
        ...
        const m = data.description.match(/\((\d{1,2})\/(\d{1,2})\/(\d{4})\)/); // (M/D/YYYY)
        if (m) {
    ...
    // Tri du plus vieux (petite date) au plus récent
    // Tri inverse: plus récent d'abord (grand sortKey en premier)
    items.sort((a,b)=> b.sortKey - a.sortKey);
    // Efface le conteneur puis réaffiche
    const articlesContainer = document.getElementById('articlesList');
...
// Fallback direct Raindrop (pas de Cloud Function)
const RAINDROP_TOKEN = '6c53f873-1caa-4b7f-84a5-f145b428d9b6'; // ⚠ Ne mets ceci que si projet privé / étudiant
...
async function syncRaindropDirect(tag){
  try {
    ...
    let items = [];
    // pagination simple jusqu'à 3 pages max (150 items)
    for (const q of baseQueries){
      for (let page=0; page<3; page++){
        ...
        if (!chunk.length) break; // plus de pages
        items.push(...chunk);
        if (chunk.length < 50) break; // dernière page
      }
      if (items.length) break; // stop si une query a rapporté des résultats
    }
    if(!items.length){ console.log('[Raindrop] Aucun item trouvé'); return; }

    // Charger URLs existantes pour anti-doublon
    ...
    for (const it of items){
      ...
      if(!link) continue;
      if(existingUrls.has(link)) { /* doublon -> skip */ continue; }
...
// Remplace l'appel Cloud Function par l'appel direct si token présent
window.addEventListener("load", async () => {
  ...
  if (RAINDROP_TOKEN && RAINDROP_TOKEN !== 'TON_TOKEN_RAINDROP_ICI') {
    await syncRaindropDirect('cyber');
  }
  chargerArticles();
});
